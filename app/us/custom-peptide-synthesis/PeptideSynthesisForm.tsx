...
export default function PeptideSynthesisForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Build a formatted message from all the peptide data
            ...
            const recaptchaToken = await getRecaptchaToken("us_peptide_submit");
            const data = {
                ...
                >
                    ...
                    {status === "error" && (
                        <div className="mx-6 mt-6 p-4 bg-red-50 text-red-700 rounded-xl border border-red-200 font-sans shadow-sm flex items-center gap-3">
                            ...
                            <div>
                                <strong>Submission failed.</strong> Please try again or email us directly at bdusa@provisbiolabs.com.
                            </div>
                        </div>
                    )}
                    ...
                    <div aria-hidden="true" style={{ position: 'absolute', left: '-9999px', top: '-9999px', height: 0, width: 0, overflow: 'hidden' }}>
                        <label htmlFor="_company_website_us_peptide">Company Website</label>
                        <input
                            type="text"
                            id="_company_website_us_peptide"
                    ...
                    <div className="p-6 sm:p-8 lg:p-10 border-b border-[#E2E8F0]">
                        ...
                        <div className="space-y-8">
                            {peptides.map((pep, idx) => (
                                <div
                                    key={pep.id}
                                    className="relative bg-[#FAFBFC] border border-[#E2E8F0] rounded-2xl p-5 sm:p-6"
                                >
                                    {/* Peptide header */}
                                    ...
                                    <div className="grid sm:grid-cols-3 gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                />
                                                <select
                                                    value={pep.quantityUnit}
                                                    onChange={(e) => updatePeptide(pep.id, "quantityUnit", e.target.value)}
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <select
                                                    value={pep.purity}
                                                    onChange={(e) => updatePeptide(pep.id, "purity", e.target.value)}
                                    ...
                                    {/* Row 2: Sequence */}
                                    <div className="mb-4">
                                        <label className="block text-[12px] font-bold text-[#475569] mb-1.5 uppercase tracking-wider">
                                            Sequence (N&apos;→C&apos;) *
                                        </label>
                                        <textarea
                                            required
                                            ...
                                            placeholder="Enter amino acid sequence using one-letter or three-letter codes, e.g. HAEGTFTSDVSSYLEG..."
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={pep.nTerminal}
                                                onChange={(e) => updatePeptide(pep.id, "nTerminal", e.target.value)}
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={pep.cTerminal}
                                                onChange={(e) => updatePeptide(pep.id, "cTerminal", e.target.value)}
                                    ...
                                    {/* Row 4: Other Modifications & Disulfide */}
                                    <div className="grid sm:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label className="block text-[12px] font-bold text-[#475569] mb-1.5 uppercase tracking-wider">Other Modifications</label>
                                            <input
                                                type="text"
                                                value={pep.otherModifications}
                                                onChange={(e) => updatePeptide(pep.id, "otherModifications", e.target.value)}
                                        ...
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={pep.disulfideBridges}
                                                onChange={(e) => updatePeptide(pep.id, "disulfideBridges", e.target.value)}
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            rows={2}
                                            value={pep.remarks}
                                            onChange={(e) => updatePeptide(pep.id, "remarks", e.target.value)}
                    ...
                    <div className="p-6 sm:p-8 lg:p-10 border-b border-[#E2E8F0] bg-[#FAFBFC]">
                        ...
                        <p className="text-[#64748B] text-sm mb-4">
                            We support 300+ peptide modifications. Here are some commonly requested modifications — specify any of these in the fields above:
                        </p>
                        <div className="flex flex-wrap gap-2">
                            {commonModifications.map((mod) => (
                                <span
                                    key={mod}
                                    className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-white border border-[#E2E8F0] text-[#475569] hover:border-[#F26522]/30 hover:text-[#F26522] transition-colors cursor-default"
                                >
                                    {mod}
                                </span>
                            ))}
                        ...
                        <div className="mt-5 p-4 bg-[#EFF6FF] rounded-xl border border-[#BFDBFE]">
                            <p className="text-xs text-[#1E3A8A] leading-relaxed">
                                <strong>Notes:</strong> (1) If not specified, amino acids will be synthesized in L-configuration by default. (2) Disulfide bridge format: <code className="bg-white/70 px-1.5 py-0.5 rounded text-[#F26522] font-mono">N2-9</code> denotes a bridge between the 2nd and 9th residues from the N-terminal. (3) For complex modifications not listed, describe them in the remarks field.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 sm:p-8 lg:p-10">
                        <button
                            ...
                            className={`group w-full bg-gradient-to-r from-[#F26522] to-[#FF8C55] hover:from-[#d95a1e] hover:to-[#F26522] text-white font-bold text-base rounded-2xl py-4.5 transition-all flex items-center justify-center gap-2.5 shadow-lg shadow-[#F26522]/20 hover:shadow-xl hover:shadow-[#F26522]/30 hover:-translate-y-0.5 ${
                        ...
                        <p className="text-center text-xs text-[#94a3b8] mt-4">
                            By submitting, you agree to our <a href="/us/privacy-policy" className="text-[#F26522] hover:underline">Privacy Policy</a>. Our team will respond within 1 business day.
                        </p>
                    </div>
                </motion.form>
            </div>
