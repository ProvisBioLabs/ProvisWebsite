...
import USGlobalSearch from './USGlobalSearch';
...
export const navItems: NavItem[] = [
        ...
        columns: [
            {
                ...
                items: [
                    ...
                    { name: 'PNGase F Flash™', href: '/us/pngase-f-flash' },
                    { name: 'Streptavidin', href: '/us/streptavidin' },
                    { name: 'Enterokinase', href: '/us/enterokinase' },
                    { name: 'Kex2 Protease', href: '/us/kex2-protease' },
                    { name: 'All Products →', href: '/us/products' },
                ],
            },
        ],
    },
    ...
    },
    {
        ...
        href: '/us/blogs',
        dropdown: [
            { name: 'Blogs', href: '/us/blogs', desc: 'Insights and articles' },
            { name: 'Events', href: '/us/events', desc: 'Conferences & launches' },
        ]
    },
    {
        label: 'About',
    ...
    },
    {
        ...
        href: '/us/contact',
        dropdown: [
            { name: 'BUSINESS ENQUIRY', href: '/us/contact', desc: 'Get in touch for business' },
            { name: 'PARTNERS', href: '/us/partners', desc: 'Explore partnerships' },
        ],
    },
];
...
const Navbar: React.FC = () => {
    ...
    const isLinkActive = (href: string) => {
        ...
        if (linkPath === '/us' || linkPath === '/') return pathname === '/us' || pathname === '/';
        ...
        <>
            <header className={`nav-header ${scrolled ? 'nav-scrolled bg-white/95 backdrop-blur-md shadow-sm' : 'bg-transparent'}`} suppressHydrationWarning>
                <div className="nav-container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" suppressHydrationWarning>
                    {/* Logo */}
                    <Link href="/us" className="nav-logo flex-shrink-0">
                        <Image
                            src="/logo.webp"
                            alt="Provis Biolabs"
                            width={160} height={44} priority
                            className={!scrolled && isDarkHeaderPage ? "brightness-0 invert opacity-90" : ""}
                        />
                    </Link>
                    ...
                    <nav className="nav-links hidden lg:flex items-center gap-5 ml-auto">
                        {navItems.map(item => (
                            ...
                            >
                                <a
                                    href={item.href}
                                    className={`nav-link text-[15px] font-sans font-normal tracking-wide flex items-center gap-1.5 transition-colors duration-200 py-6 ${activeMenu === item.label || isItemActive(item)
                                            ? 'text-[#F26522]'
                                            : (!scrolled && isDarkHeaderPage ? 'text-white/90 hover:text-white' : 'text-[#475569] hover:text-[#F26522]')
                                        }`}
                                >
                                    {item.label}
                                        ...
                                        <div className="bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] border-t border-[#f1f5f9] relative overflow-hidden">
                                            ...
                                            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex gap-12 justify-between">
                                                ...
                                                <div className="w-[320px] bg-[#f8fafc] rounded-2xl p-8 border border-[#f1f5f9] flex flex-col justify-center relative overflow-hidden group/promo cursor-pointer hover:border-[#F26522]/30 transition-colors">
                                                    ...
                                                    <Link href="/us/cdmo" className="text-[#F26522] font-sans text-[14px] font-bold flex items-center gap-2 group-hover/promo:gap-3 transition-all">
                    ...
                    {/* Mobile Hamburger */}
                    <button
                        className={`lg:hidden p-2 hover:bg-black/10 rounded-lg transition-colors ml-auto ${!scrolled && isDarkHeaderPage ? 'text-white' : 'text-[#1E3A8A]'}`}
