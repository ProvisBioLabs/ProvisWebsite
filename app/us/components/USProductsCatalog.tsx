...
import { products as allProducts, Product } from "../../../lib/data/usProducts";

// No categories, just show recombinant bio-reagents
...
function ProductCard({
    ...
    return (
        ...
        >
            {/* Clickable Overlay */}
            <Link href={`/us${product.href}`} className="absolute inset-0 z-10 rounded-2xl">
            ...
            <div className="w-full h-52 rounded-xl flex items-center justify-center mb-6 relative overflow-hidden border border-[#E2E8F0] group-hover:border-[#F26522]/20 transition-colors z-0">
                <img
                    src={product.image}
                    alt={`${product.name} — Recombinant bio-reagent by Provis Biolabs`}
...
export default function USProductsCatalog() {
    const [searchQuery, setSearchQuery] = useState("");

    // Categorize products for the UI - only reagents
    const categorizedProducts = useMemo(() => {
        return allProducts.filter(p => p.category === 'reagent');
    }, []);
                        ...
                        >
                            <div>
                                <p className="text-sm font-semibold text-[#475569]">
                                    GMP-grade recombinant proteins and enzymes for bioprocessing, cell culture and biomanufacturing.
                                </p>
                            </div>
